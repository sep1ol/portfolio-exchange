...
async function main() {
    ...
    console.log(`Transferred ${format(amount)} mETH\n**from ${user1.address}\n**to ${user2.address}\n`)
    ...
    await transaction.wait()
    transaction = await exchange.connect(user1).depositToken(sep1ol.address,amount)
    await transaction.wait()
    console.log(`>> Deposited ${format(amount)} Sepiol Tokens\n**from ${user1.address}`)
    ...
    await transaction.wait()
    console.log(`>> Deposited ${format(amount)} mEthereum\n**from ${user2.address}`)
    ...
    transaction = await exchange.connect(user1).makeOrder(ethAddress, tokens(50), sepiolAddress, tokens(15))
    result = await transaction.wait()
    ...
    transaction = await exchange.connect(user1).makeOrder(ethAddress, tokens(200), sepiolAddress, tokens(20))
    result = await transaction.wait()
